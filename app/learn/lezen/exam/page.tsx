...
export default function LezenExamPage() {
  ...
  // Get all questions from all passages, shuffled
  const examData = useMemo(() => {
    const allData = getLezenExamQuestions();
  ...
  const currentPassage = examData[currentQuestionIndex]?.passage;
  ...
  if (!started) {
    return (
      <ExamIntroScreen
        title="Lezen A2 Oefenexamen"
        questionCount={questions.length}
        recommendedTime="35 minuten"
